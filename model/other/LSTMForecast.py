...
model.add(LSTM(150, activation='relu', input_shape=(n_steps, n_features), kernel_regularizer=l2(0.01)))
model.add(Dropout(0.2))
model.add(Dense(48))
model.add(Dropout(0.2))
...
#save model
model.save("forecastusefinalmodel9.h5")

y_pred = model.predict_classes(x, batch_size=batchSize, verbose=0)
# y_pred = model.predict_classes(x, batch_size=batchSize, verbose=0)
print('predicted', y_pred[222])
# print(Y_test[1])
# print(y[1])
y_labels = np.argmax(y, axis=1)
# y_labels = np.argmax(y, axis=1)
print('labels: ',y_labels[222])#222
...
model = load_model('forecastusefinalmodel9.h5')
